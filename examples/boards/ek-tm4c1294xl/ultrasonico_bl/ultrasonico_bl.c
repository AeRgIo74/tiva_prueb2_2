...
void UARTSend(const uint8_t *pui8Buffer, uint32_t ui32Count) {
    ...
    while (ui32Count--) {
        while (!UARTCharPutNonBlocking(UART6_BASE, *pui8Buffer++)) {
            // Esperar hasta que haya espacio en el FIFO
            SysCtlDelay(ui32SysClock / 3000);  // Ajusta el valor según sea necesario
        }
    }
}
...
void UART6IntHandler(void) {
    ...
    while (UARTCharsAvail(UART6_BASE)) {
        ...
        UARTCharPutNonBlocking(UART6_BASE, receivedChar);
    }
}
...
int main(void)
{
    ...
    user_led_N();
    buzzer_E();
    char buffer4[10];  // Buffer para almacenar la distancia como cadena
    while (1)
    {
        distancia();
        if(distance <= 7){
        ...
        // Convertir la distancia a cadena
        IntToChar(distance, buffer4);
        
        // Enviar la distancia por UART
