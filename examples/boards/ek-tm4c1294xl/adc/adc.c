...
int main(void) {
    ...
    ADCSequenceConfigure(ADC0_BASE, 0, ADC_TRIGGER_TIMER, 0); // Secuencia 0
    ADCSequenceStepConfigure(ADC0_BASE, 0, 0, ADC_CTL_CH0 | ADC_CTL_END | ADC_CTL_IE); // Canal 0
    ADCSequenceEnable(ADC0_BASE, 0); // Habilitar secuencia 0
    ADCIntClear(ADC0_BASE, 0); // Limpiar interrupción

    // Configurar Timer 0 para generar la señal periódica
    TimerConfigure(TIMER0_BASE, TIMER_CFG_PERIODIC); // Configurar timer como periódico
    TimerLoadSet(TIMER0_BASE, TIMER_A, (g_ui32SysClock / 16000) - 1); // 16 kHz
    TimerControlTrigger(TIMER0_BASE, TIMER_A, true); // Controlar el disparador del Timer
